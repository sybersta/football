...
//    	getOriginalPosition(new MongoRaceEntity(), " 192.168.31.4",12);
    ...
    public static MongoRaceEntity getOriginalPosition(MongoRaceEntity m, String ip, int count) {
        System.out.println(m.getRaceId() + ":start");
...
//        httpget.addHeader("Cookie", "race_id="+m.getRaceId()+"; uid=R-590961-283ea6710610792b0685af; ds_session=b7erh0uug949a7teli3nepku36; Hm_lvt_a68414d98536efc52eeb879f984d8923=1627886171,1627959604; cf_chl_rc_m=1; cf_chl_2=7d527b0e582eb6b; cf_chl_prog=a11; cf_clearance=bc0f1f635abcfbd9bcf20629e46f438b00586463-1627962212-0-150; __cf_bm=0e47bf8c713fdcbcff0c38317e1b6bdb403a773d-1627962275-1800-AfQxKnZfZzDokxdjnQGZSx291uPMYGMf34xlriF1z7fxh/9JNHI43tq87MA8QmViOln4xUpLpOVNSgOf/p6K37L9PjTU6+OESb7Lqlgye+/cGvq2CoA2MZEB0+Djkhde1g==; Hm_lpvt_a68414d98536efc52eeb879f984d8923=1627962259");
        ...
        try {
            ...
            return getOriginalPosition(m,ip,1);
        }
    ...
    public static MongoRaceEntity setPankouValue(Document doc, MongoRaceEntity m) {
...
//获取初始盘口
            while(true) {
                try{
                tr2 = ((Element)tr.get(tr1 - 1)).getElementsByTag("td");
                if(tr2 == null || tr2.size() <= 0) {
                    break;
                }

                if(!((Element)tr2.get(0)).text().contains("-")) {
                    tr2 = ((Element)tr.get(tr1)).getElementsByTag("td");
                    if(tr2 != null && tr2.size() > 0 && !((Element)tr2.get(3)).text().contains("-")) {
                        tr3 = ((Element)tr2.get(3)).text().split(",");
                        ...
                        m.setHandicap(td);
                        m.setHomeOdds(getVal(((Element)tr2.get(2)).text()));
                        m.setVisitingOdds(getVal(((Element)tr2.get(4)).text()));
                    }
                    break;
                }
    ...
    public static MongoRaceEntity setDaxiaoValue(Document doc, MongoRaceEntity m) {
        ...
        if(tr != null && tr.size() > 0) {
            ...
            while(true) {
                ...
                if(!((Element)tr2.get(0)).text().contains("-")) {
                    ...
                    if(tr2 != null && tr2.size() > 0 && !((Element)tr2.get(3)).text().contains("-")) {
                        tr3 = ((Element)tr2.get(3)).text().split(",");
                        Double td = getVal(tr3[0]);
                        if(tr3.length > 1) {
                            td = (getVal(tr3[0]) + getVal(tr3[1])) / 2.00D;
                        }

                        m.setGoalLine(td);
                        m.setUpperOdds(getVal(((Element)tr2.get(2)).text()));
                        m.setLowerOdds(getVal(((Element)tr2.get(4)).text()));
//                        m.setObn(PankouUtils.getBigSmallPosition(getVal(((Element)tr2.get(2)).text()), td, (getVal(((Element)tr2.get(2)).text()) + getVal(((Element)tr2.get(4)).text())) / 2.00D));
                    }
                    break;
                }

                --tr1;}catch (Exception ex){
                    break;
                }
            }
            int num=1;
    ...
    public static MongoRaceEntity setSpfValue(Document doc, MongoRaceEntity m) {
//        Element rf = doc.getElementById("sp_bet");
        ...
        if(tr != null && tr.size() > 0) {
            ...
            while(true) {
                ...
                if(!((Element)tr2.get(0)).text().contains("-")) {
                    tr2 = ((Element)tr.get(tr1)).getElementsByTag("td");
                    if(tr2 != null && tr2.size() > 0 && !((Element)tr2.get(2)).text().contains("-")) {
                        m.setWinOdds(getVal(((Element)tr2.get(2)).text()));
                        m.setDrawOdds(getVal(((Element)tr2.get(3)).text()));
                        m.setLostOdds(getVal(((Element)tr2.get(4)).text()));
                    }
                    break;
                }
                --tr1;}catch (Exception ex){
